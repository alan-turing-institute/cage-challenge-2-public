...
class BlueTableWrapper(BaseWrapper):
    ...
    def step(self, agent=None, action=None) -> Results:
        ...
        obs = result.observation
        if agent == 'Blue':
            changed_obs = self.observation_change(obs)
        if 'success' in obs.keys():
            if obs['success'].name == 'TRUE':
                obs = np.append([1, 0], changed_obs)
            elif obs['success'].name == 'FALSE':
                obs = np.append([0, 1], changed_obs)
            elif obs['success'].name == 'UNKNOWN':
                obs = np.append([0, 0], changed_obs)
            else:
