...
class NewBlueTableWrapper(BaseWrapper):
    def __init__(self,env=None,agent=None,output_mode='table', port_length=12):
    ...
    def observation_change(self,observation,baseline=False):
        ...
        elif self.output_mode == 'float_vector':
            return self._create_float_vector(success, port_length=self.port_length)
        else:
    ...
    def _process_initial_obs(self, obs):
        ...
        for hostid in obs:
            ...
            self.blue_info[hostname] = [str(subnet),str(ip),hostname, 'None', 'None','No'] # [-3] is local_ports
    ...
    def _process_anomalies(self,anomaly_dict):
        ...
        for hostid, host_anomalies in anomaly_dict.items():
            # print('info[hostid]', info[hostid])
            assert len(host_anomalies) > 0
            if 'Processes' in host_anomalies:
                connection_type, all_ports = self._interpret_connections(host_anomalies['Processes'])
                info[hostid][-2] = connection_type
                info[hostid][-3] = all_ports
    ...
    def _interpret_connections(self,activity:list):                
        ...
        all_ports = [item['Connections'][0]['local_port'] \
            for item in activity if 'Connections' in item]
        ...
        if num_connections >= 3 and port_focus >=3:
            ...
            anomaly = 'Scan' 
        str_all_ports = ','.join([str(port) for port in ports])
        return anomaly, str_all_ports
    ...
    def _create_float_vector(self, success, port_length):
        ...
        for row in table:
            ...
            if ports != 'None':
                try:
                    fill_ports = [float(port) for port in ports.split(',')] 
                except: 
                    fill_ports = []

                if len(fill_ports) <= port_length:
                    all_ports = fill_ports + [0.]*(port_length-len(fill_ports))
                else:
                    all_ports = fill_ports[:port_length]
            elif ports == 'None':
                all_ports = [0.]*port_length
            else: 
                raise ValueError('Table had invalid Access Level-- ports')
            proto_vector.extend(all_ports)
        ...
        success_type = success.value
        if success_type == 1: # true
            value = [0.]
        elif success_type == 2: 
            value = [1.]
        elif success_type == 3:
            ...
            raise ValueError('Table had invalid Access Level-- success value unseen')
        proto_vector.extend(value)
