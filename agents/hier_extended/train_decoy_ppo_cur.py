...
from CybORGDecoy import CybORGDecoyAgent
...
if __name__ == "__main__":
    ...
    stop = {
        ...
        "timesteps_total": 3000000,   # Total number of timesteps
    ...
    analysis = tune.run(algo,  # Algo to use - alt: ppo.PPOTrainer, impala.ImpalaTrainer
                        config=config,
                        name=algo.__name__ + '_decoy_' + CybORGDecoyAgent.agents['Red'].__name__ + '_' + time.strftime("%Y-%m-%d_%H-%M-%S"),
                        local_dir=log_dir,
                        stop=stop,
                        #restore=checkpoint,
